...
function Clock() {
    ...
    const start = () => {
        ...
        stop();
        const idForTimer: number = +setInterval(()=>{
            setDate(new Date())
        }, 1000)
        setTimerId(idForTimer)
    }
    ...
    //const dayNumber = date.getDate().toString().length < 2 ? '0'+(date.getDate()).toString() : (date.getDate().toString());
    //const monthNumber = (date.getMonth()+1).toString().length < 2 ? '0'+(date.getMonth()+1).toString() : (date.getMonth().toString() +1);
    //const stringDate = dayNumber + '.' + monthNumber + '.' + date.getFullYear() || <br/> // день.месяц.год (01.02.2022) // пишут студенты, варианты 01.02.0123/01.02.-123/01.02.12345 не рассматриваем
    ...
    const stringDay = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][date.getDay()] || <br/> // пишут студенты
